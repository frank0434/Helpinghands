...
build_factor_dropdowns <- function(factor_df,type,no, pool = NULL) {
  ...
  if(!("RCP" %in% factors)){
  	...
  	for(k in 1:length(controls)){
	  	...
	  	cmb <- pickerInput(
	  		...
	  		choices = c(factor_choices),
	  		selected = 1,options = list(`actions-box` = TRUE),multiple = T)
	  	dropdown_list[[k]] <- cmb
  ...
  for(i in seq_along(factors)) {
    ...
    cmb <- pickerInput(
      ...
      label = paste0(factor," #",no),
      choices = c(factor_choices),
      selected = factor_choices[1],options = list(`actions-box` = TRUE),multiple = T)
...
createParameterMap <- function(vcsnStations) {
	
	leaflet() %>%
		addTiles() %>%
		fitBounds(min(vcsnStations$LONGT), min(vcsnStations$LAT), max(vcsnStations$LONGT), max(vcsnStations$LAT)) %>%
		addDrawToolbar(
...
#function taken from https://github.com/RedOakStrategic/geoshaper
