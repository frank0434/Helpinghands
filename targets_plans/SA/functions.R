...
#'
#' @examples
model_richness <- function(data, Spp.Rich.coeff,  
                           name_predictor = "predictor"){
    richness <- data %>% 
      # join the coefs
      right_join(Spp.Rich.coeff, by = name_predictor) %>% 
...
#' @param dat the dependencies data
#'
#' @return
#' @export
#'
#' @examples
model_richness_SA <- function(data, coefs, 
                               # model 2 required
                               AllCDIs = data_CDI,
                               site_name = "AND",
                               name_site =  "Site_Code",
  ...
  #nest the data.table for richness calculation
  coefs <- coefs[, list(richness = list(.SD)), by = .(ID)]
  # richness for one model 
  if(multi_models==FALSE){

    y <- coefs[, .(lapply(richness, function(x){
      richness <- model_richness(data , Spp.Rich.coeff = x,  
                                 name_predictor = "predictor") 
      })), by = .(ID)][, unlist(V1, recursive = FALSE), by = .(ID)]
    return(y$richness)
    # richness for multi models
    } else {
      yyy <- coefs[, .(lapply(richness, function(x){
        richness <- model_richness(data , Spp.Rich.coeff = x,  
                                   name_predictor = "predictor")
        litter_decomposition_rate <- decomposition(taxonomic.richness = richness,
                                                   AllCDIs, name_site )
        litterMass <- surfaceOM(Weather,
...
surfaceOM <- function(Weather, decomposition, 
  ...
  sites <- unique(decomposition[[name_site]])
