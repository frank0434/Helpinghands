...
# simulated data ----------------------------------------------------------
  ...
  tar_target(data, 
             tibble::tibble(
              ...
              Site_Code = site_name) %>% 
               as.data.table() %>% 
               melt.data.table(variable.name = "predictor", 
                               variable.factor = FALSE)
  ),
# Sensitivities in one site 
  tar_target(dat, data[Site_Code == "AND"]),
  ...
  tar_target(DT_mat, data.table(mat)[, ID := 1:.N] %>% 
               melt.data.table(id.vars = c("ID"), 
                               variable.factor = FALSE,
                               variable.name = "predictor",
                               value.name = "coef")),
  tar_target(y, model_richness_SA(data = dat, coefs = DT_mat)),
  tar_target(yyy, model_richness_SA(data = dat, coefs = DT_mat,
                                     AllCDIs = data_CDI[1,],
                                     site = site_name[1],
                                     # model 3 required 
